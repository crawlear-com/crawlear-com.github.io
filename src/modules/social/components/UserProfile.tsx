...
import { User } from 'src/modules/social/User'
...
function UserProfile({user, onLogout}: UserProfileProps) {
    ...
    const followAction: Array<React.JSX.Element> = []

    React.useEffect(()=>{
        if (isUserLoged && !isTheUserLogged(user.uid)) {
            fbBase.getFidFromFollow(window.crawlear.user.uid, user.uid, (result)=>{
                setFid(result)
            }, ()=>{})
        }
    },[isUserLoged])
        ...
        <div className="userProfileInlineContainer">
            ...
            <div className='instagram'>
                ...
                {isTheUserLogged(user.uid) ? <input type="text" 
                    ...
                    instagram ? 
                        <a href={`https://www.instagram.com/${instagram}/`} rel="noreferrer" target="_blank">@{instagram}</a> : <></>}
            </div> : <></>}
